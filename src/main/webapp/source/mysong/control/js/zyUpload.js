	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm" action="uploadMusic.do" method="post" enctype="multipart/form-data" style="height:400px; position:relative;">';
					html += '<div style="display:block; font-size:14px;" >歌曲名称：<input id="musicName" class="text1" type="text" style="width:200px;height:30px;" name="musicName"></div><br>';
					html += '<div style="display:block; font-size:14px; margin-left:27px;" >原唱：<input id="special" class="text1" type="text" style="width:200px;height:30px;" name="special"></div><br>';
					html += '<div style=" display:block; font-size:14px;" >音乐类型：<select class="text1" style="width:205px;height:30px;" name="musicType"> <option value="流行">流行</option><option value="民谣">民谣</option><option value="嘻哈">嘻哈</option><option value="摇滚">摇滚</option><option value="R&B">R&B</option><option value="乡村">乡村</option><option value="中国传统">中国传统</option></select></div>';				
					html += '	<div class="upload_box">';
					html += '		<div class="upload_main">';	
					html += '			<div class="upload_choose" style="margin-top:-20px">';
		            ...
		            html += '				<div id="status_info" class="info">文件大小共0KB。</div>';
		            html += '				<div class="btns">';
					...
					html += '			<button type="sumbit" id="fileSubmit" class="upload_submit_btn">确认上传文件</button>';
					html += '		</div>';
					...
					// 创建不带有拖动的html
					html += '<form id="uploadForm" action="uploadMusic.do" method="post" enctype="multipart/form-data">';
					html += '<div style="display:block; font-size:14px;" >歌曲名称：<input id="musicName" class="text1" type="text" style="width:200px;height:30px;" name="musicName"></div><br>';
					html += '<div style="display:block; font-size:14px;" >原唱：<input id="special" class="text1" type="text" style="width:200px;height:30px;" name="special"></div><br>';
					html += '<div style="display:block; font-size:14px;"  >音乐类型：<select style="width:200px;height:30px;" class="text1" name="musicType" style="width:200px;height:30px;"> <option style="width:200px;height:30px;" value="1">流行</option><option style="width:200px;height:30px;" value="2">民谣</option><option style="width:200px;height:30px;" value="3">嘻哈</option><option value="4">摇滚</option><option value="5">R&B</option><option value="6">乡村</option><option value="7">中国传统</option></div><br>';
					html += '	<div class="upload_box">';
					html += '		<script>';
					html += '		function hp(){if($("#musicName").value()==null){alert("请填写歌曲名")}}';
					html += '		</script>';
				    ...
				    html += '							<img class="upload_image" src="add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funFilterEligibleFile = function(files){
				...
				//if(files[0].type.indexOf("mp3")<=0){
						//alert('文件格式错误');	
				//}else{
				for (var i = 0, file; file = files[i]; i++) {
					//alert(files[i].type.indexOf("mp"));
					if(files[i].type.indexOf("mp")<=0){
						alert('文件格式错误');	
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '		<div class="uploadImg" style="width:'+imgWidth+'px">';				
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="source/mysong/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';                                                                 
					html += '		</div>';
					html += '	</a>';
					...
					html += '		</div>';
					html += '	</div>';
					html += '	<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';"  class="imgBox">';
					html += '		<div class="uploadImg" style="width:'+imgWidth+'px">';				
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="source/mysong/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';                                                                 
					html += '		</div>';
					html += '	</a>';
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					// 判断当前是否有文件需要上传
					var i = $(this).parent().parent().parent().parent().parent().find("#musicName").val();
					var j = $(this).parent().parent().parent().parent().parent().find("#special").val();
					if(i==null||i==""){						
						alert("请输入歌曲名");
					}else if(j==null||j==""){
						alert("请输入原唱歌手");
